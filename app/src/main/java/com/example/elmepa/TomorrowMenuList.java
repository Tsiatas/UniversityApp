...
public class TomorrowMenuList extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_tomorrow_menu_list);
        try {
            setFood();
        }
        catch(Exception e){
    ...
    public int findMonth() {
        ...
        numOfMonth = Integer.parseInt(formattedDate);
    ...
    public int findDayOfMonth() {
        int numOfMonth;
        Date date = Calendar.getInstance().getTime();
        @SuppressLint("SimpleDateFormat") SimpleDateFormat dateFormat = new SimpleDateFormat("ddd");
        String formattedDate = dateFormat.format(date);
        numOfMonth = Integer.parseInt(formattedDate);
        return numOfMonth;
    ...
    void setFood() {
        ...
        int numOfDay = findDay();
        numOfDay++;
        if (findMonth() == 2 && findDayOfMonth() == 28) {
            if (numOfDay == 8) {
                numOfDay = 1;
                numOfWeek++;
            }
        } else if ((findMonth() == 1 || findMonth() == 3 ||findMonth() == 5 ||findMonth() == 7 || findMonth() == 8 || findMonth() == 10 || findMonth() == 12) && findDayOfMonth() == 31) {
            if (numOfDay == 8) {
                numOfDay = 1;
                numOfWeek++;
            }
        } else if ((findMonth() == 4 || findMonth() == 6 || findMonth() == 9 || findMonth() == 11) && findDayOfMonth() == 30) {
            if (numOfDay == 8) {
                numOfDay = 1;
                numOfWeek ++;
            }
